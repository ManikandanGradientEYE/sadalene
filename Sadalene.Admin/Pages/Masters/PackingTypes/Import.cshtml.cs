...
public class ImportModel : PageModel
{
    ...
    public async Task<IActionResult> OnGetTemplateAsync()
    {
        ...
        var ws = wb.Worksheets.Add("PackingTypes");
        ...
        ws.Cell(2, 1).Value = "Box Packing";
        ws.Cell(2, 2).Value = "Packed in a carton box";
        ...
        return File(stream.ToArray(),
            ...
            "PackingType-Import-Template.xlsx");
    }
    ...
    public async Task<IActionResult> OnPostAsync(IFormFile? file)
    {
        ...
        var pending = new List<PackingType>();
        ...
        for (int row = 2; row <= lastRow; row++)
        {
            ...
            if (!namesInFile.Add(name) || await _db.PackingTypes.AnyAsync(p => p.Name == name))
            {
                Errors.Add($"Row {row}: Packing type '{name}' already exists.");
                continue;
            }

            pending.Add(new PackingType { Name = name, Description = description });
        }

        if (pending.Count == 0)
        {
            if (Errors.Count == 0) Errors.Add("No packing type rows found in the uploaded file.");
            return Page();
        }

        _db.PackingTypes.AddRange(pending);
        try
        {
        ...
        {
            Errors.Insert(0, "Import failed — no packing types were saved. " +
        ...
        TempData["Success"] = $"Imported {ImportedCount} packing type(s).";
        return Page();
    }
}
